...
function en_hijri_calendar() {
    include "calendar_generator.php";
}
...
function en_hijri_date() {
    ...
    $options = get_option("hcal_options");
    if (!is_array($options)) {
        $options = array( 'time_zone' => 'Asia/Dhaka', 'adjust_hour' => '24', 'language' => 'en', 'format' => '1' );
    }
    ...
    if ($options['language'] == "en") {
        if ($options['format'] == "1") {
            echo hcal_lang_rplc($d->date("jS F, Y", time()-$offset)) . " A.H.";
        }
        elseif ($options['format'] == "2") {
            echo hcal_lang_rplc($d->date("jS F, Y", time()-$offset));
        }
        elseif ($options['format'] == "3") {
            echo hcal_lang_rplc($d->date("F jS, Y", time()-$offset));
        }
        elseif ($options['format'] == "4") {
            echo hcal_lang_rplc($d->date("Y F jS", time()-$offset));
        }
        elseif ($options['format'] == "5") {
            echo hcal_lang_rplc($d->date("j F Y", time()-$offset));
        }
        elseif ($options['format'] == "6") {
            echo hcal_lang_rplc($d->date("d F Y", time()-$offset));
        }
        elseif ($options['format'] == "7") {
            echo hcal_lang_rplc($d->date("Y F j", time()-$offset));
        }
        elseif ($options['format'] == "8") {
            echo hcal_lang_rplc($d->date("Y F d", time()-$offset));
        }
        elseif ($options['format'] == "9") {
            echo hcal_lang_rplc($d->date("d-m-Y", time()-$offset));
        }
        elseif ($options['format'] == "10") {
            echo hcal_lang_rplc($d->date("Y-m-d", time()-$offset));
        }
        else {
            echo hcal_lang_rplc($d->date("jS F, Y", time()-$offset)) . " A.H.";
        }
    ...
    elseif ($options['language'] == "custom") {
        if ($options['format'] == "1") {
            echo hcal_lang_rplc($d->date("j F, Y", time()-$offset)) . " A.H.";
        }
        elseif ($options['format'] == "2") {
            echo hcal_lang_rplc($d->date("j F, Y", time()-$offset));
        }
        elseif ($options['format'] == "3") {
            echo hcal_lang_rplc($d->date("F j, Y", time()-$offset));
        }
        elseif ($options['format'] == "4") {
            echo hcal_lang_rplc($d->date("Y F j", time()-$offset));
        }
        ...
        elseif ($options['format'] == "10") {
            ...
            echo hcal_lang_rplc($d->date("d F, Y", time()-$offset)) . " A.H.";
        }
    }
...
function widget_hijri_calendar($args) {
    ...
    $options = get_option("hcal_options");
    if (!is_array($options)) {
        $options = array( 'wgt_title' => 'Hijri Calendar' );
    } ?>
