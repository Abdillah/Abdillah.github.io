	...
	tinymce.create('tinymce.plugins.CodeElementPlugin', {
		init : function(ed, url) {
			...
			ed.addCommand('mceCodeElement', function() {
				...
				if (e===null){
					var sel_tx = ed.selection.getContent();
					prefix = sel_tx.match(/^[\s]*/)[0];
					suffix = sel_tx.match(/[\s]*$/)[0];
					content = sel_tx.replace(/^[\s]*/, '').replace(/[\s]*$/, '');
					window.console.log(prefix + ":" + suffix);
					//add code element
					if ( ed.selection.isCollapsed() ) {
						ed.execCommand('mceInsertContent', false, '<code>code</code>&nbsp;');
					} else {
						ed.selection.setContent(prefix +'<code>' + content + '</code>' + suffix);
					}
				} else {
					//remove code element
